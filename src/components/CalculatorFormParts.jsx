...
// --- 1. SEKCJA DANYCH OSOBOWYCH ---
export const PersonalSection = (props) => {
  const { wiek, setWiek, wiekEmerytura, setWiekEmerytura, rok, setRok, dostepneLata, podatek, setPodatek, czyFirma, setCzyFirma, iloscWplatRok, setIloscWplatRok, errors, setErrors } = props;

  const handleAgeChange = (val) => {
    const num = val === '' ? '' : Number(val);
    setWiek(num);
    setErrors(prev => ({ ...prev, wiek: validateAge(num) }));
  };

  const handleRetirementChange = (val) => {
    const num = val === '' ? '' : Number(val);
    setWiekEmerytura(num);
    setErrors(prev => ({ ...prev, wiekEmerytura: validateRetirementAge(num) }));
  };
    ...
    <div style={cardStyle}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '15px' }}>
        <div>
          ...
          <input type="number" value={wiek || ''} onChange={(e) => handleAgeChange(e.target.value)} style={inputStyle(errors.wiek)} />
          {errors.wiek && <div style={errorMsgStyle}>{errors.wiek}</div>}
        </div>
        <div>
          <label style={labelStyle}>Wiek emerytury</label>
          <input type="number" value={wiekEmerytura || ''} onChange={(e) => handleRetirementChange(e.target.value)} style={inputStyle(errors.wiekEmerytura)} />
...
// --- 2. SEKCJA WPŁAT (To tutaj był błąd - upewnij się, że masz ten fragment!) ---
export const ContributionsSection = (props) => {
  const { tryb, wspolnaKwota, setWspolnaKwota, limitWspolny, wplataIKE, setWplataIKE, limitIKE, wplataIKZE, setWplataIKZE, limitIKZE } = props;

  const handleNumericChange = (val, setter, maxLimit) => {
  ...
  if (tryb === 'porownanie') {
    return (
      <div style={{ ...cardStyle, background: '#ebf8ff', borderColor: '#bee3f8' }}>
        ...
        <input type="range" min="0" max={limitWspolny} step="1" value={wspolnaKwota} onChange={(e) => setWspolnaKwota(Number(e.target.value))} style={{ width: '100%', cursor: 'pointer', accentColor: '#3182ce', marginBottom: '15px' }} />
        <div style={{ display: 'flex', gap: '10px' }}>
           <input type="number" value={wspolnaKwota || ''} onChange={(e) => handleNumericChange(e.target.value, setWspolnaKwota, limitWspolny)} style={inputStyle(false)} />
           <button onClick={() => setWspolnaKwota(limitWspolny)} style={{ background: '#3182ce', color: '#fff', border: 'none', borderRadius: '8px', padding: '0 20px', fontWeight: 'bold', cursor: 'pointer' }}>MAX</button>
        </div>
        <div style={{ fontSize: '11px', color: '#5a67d8', marginTop: '10px', textAlign: 'center' }}>Limit ograniczony do: {limitWspolny.toLocaleString()} zł</div>
      </div>
    );
  }
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '15px', marginBottom: '20px' }}>
      <div style={{ ...cardStyle, background: '#f7fafc', borderColor: '#cbd5e0', marginBottom: 0 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '5px' }}><strong style={{ color: '#2b6cb0' }}>IKE</strong><span style={{ fontSize: '11px', color: '#718096' }}>Max: {limitIKE}</span></div>
        <input type="number" value={wplataIKE || ''} onChange={(e) => handleNumericChange(e.target.value, setWplataIKE, limitIKE)} style={inputStyle(false)} />
      </div>
      <div style={{ ...cardStyle, background: '#f7fafc', borderColor: '#cbd5e0', marginBottom: 0 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '5px' }}><strong style={{ color: '#276749' }}>IKZE</strong><span style={{ fontSize: '11px', color: '#718096' }}>Max: {limitIKZE}</span></div>
        <input type="number" value={wplataIKZE || ''} onChange={(e) => handleNumericChange(e.target.value, setWplataIKZE, limitIKZE)} style={inputStyle(false)} />
      </div>
    </div>
  );
};
...
// --- 3. SEKCJA RYNKOWA ---
export const MarketSection = (props) => {
  const { stopaZwrotu, setStopaZwrotu, reinwestuj, setReinwestuj, stopaZwrotuUlga, setStopaZwrotuUlga, oplatyIKE, setOplatyIKE, oplatyIKZE, setOplatyIKZE, warnings, setWarnings } = props;
    ...
    <div style={cardStyle}>
       <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px' }}>
          <div>
            ...
            <input type="number" step="0.5" value={stopaZwrotu || ''} onChange={(e) => handlePercentageChange(e.target.value, setStopaZwrotu, 'zwrot')} style={inputStyle(false)} />
            ...
            <div style={{ marginTop: '15px' }}>
              ...
              {reinwestuj ? (
                <div style={{ marginTop: '8px', paddingLeft: '24px' }}>
                  ...
                  <input type="number" step="0.5" value={stopaZwrotuUlga || ''} onChange={(e) => handlePercentageChange(e.target.value, setStopaZwrotuUlga, 'ulga')} style={{ ...inputStyle(false), padding: '6px', fontSize: '13px' }} />
                  {warnings.ulga && <div style={warningMsgStyle}>{warnings.ulga}</div>}
                </div>
              ) : (
                 <div style={{ marginTop: '8px', paddingLeft: '24px', fontSize: '11px', color: '#a0aec0' }}>Zwrot podatku trafia do kieszeni</div>
              )}
            </div>
          </div>
